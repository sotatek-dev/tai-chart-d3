...
const Line = ({ scales, data }) => {
  ...
  const path = <path d={line(data)} stroke="rgb(149 218 137 / 90%)" strokeWidth="3px" fill="none" />;
  return <g>{path}</g>;
};
// Area Component
const Area = ({ scales, data, svgDimensions, margins }) => {
  ...
  const areaGradient = (
    <linearGradient id="area-gradient" gradientUnits="userSpaceOnUse" x1="0" y1={yScale(0)} x2="0" y2={yScale(1000)}>
      <stop offset="0%" stopColor="#333333" stopOpacity="0" />
      <stop offset="100%" stopColor="rgb(149 218 137 / 90%)" stopOpacity="0.5" />
    </linearGradient>
  );
...
const Charts = () => {
  ...
  const [lineChartData] = React.useState([
    { year: 2012, income: 40 },
    { year: 2013, income: 80 },
    { year: 2014, income: 50 },
  ...
  const [barChartData, setBarChartData] = React.useState(initBarData);
  const handleChangeYear = (year) => {
    let getData = initBarData;
    if (year === "2012") {
      getData = [
        { month: 1, income: 30 },
        { month: 2, income: 40 },
